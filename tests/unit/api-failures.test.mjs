...
function forecastFetch(overrides = {}) {
  return async (input) => {
    ...
    if (url.includes("air-quality-api")) return overrides.cams ?? json(camsPayload());
...
for (const source of ["airbkk", "cams"]) {
  test(`${source} timeout returns unavailable without demo values`, async () => {
    const timeout = Promise.reject(new DOMException("timed out", "TimeoutError"));
    const payload = await (await createForecastResponse({ fetchImpl: forecastFetch({ [source]: timeout }), now: () => NOW })).json();
    assert.equal(payload.status, "unavailable");
    assert.equal(payload.dataQuality.upstream[source], "timeout");
...
test("malformed AirBKK, HTTP 500, and partial CAMS coverage map to explicit states", async () => {
  ...
  assert.equal(http500.status, "unavailable");
...
test("a genuinely hanging PM upstream is aborted within the configured timeout", async () => {
  ...
  const payload = await (await createForecastResponse({ fetchImpl: hanging, now: () => NOW, timeouts: { airbkk: 20, cams: 20, weather: 20 } })).json();
  assert.equal(payload.status, "unavailable");
